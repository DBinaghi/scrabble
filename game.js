...
var LETTERS_PLAYED_BY_AI_INDEXES = [];
...
var AI_INTELLIGENCE = 1;
var AI_MAX_INTELLIGENCE = 0.2;
...
function printPlayersLetters() {
	document.getElementById("player_1_letters").innerHTML = getPlayersLetters();
}
	...
	for (var i = 0; i < 15; i++) {
		for (var j = 0; j < 15; j++) {
			...
			if (LETTERS_PLAYED_BY_AI_INDEXES.indexOf(i * 15 + j) !== -1) {
				if (!field.classList.contains('ai_set_tile')) {
					field.classList.add('ai_set_tile');
				}
			} else {
				field.classList.remove('ai_set_tile');
			}
		}
	}
...
function setAiMaxStrength(src) {
	AI_MAX_INTELLIGENCE = src.value;
}
...
function startAiMove() {
	...
	setTimeout(
		function() {
			// ai_intelligence: the closer to 1 the more clever the a.i.
...
function computerMove() {
	...
	for (var i in MAX_RESULT) {
		LETTERS_PLAYED_BY_AI_INDEXES.push(parseInt(i));
